...
export default function ProductDetail({ product }: { product: MerchProduct }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
      {/* ── Gallery ─────────────────────────────────────────────────────── */}
      <div>
        <div className="relative aspect-square bg-surface-alt border border-accent/12 overflow-hidden">
          ...
          <div className="flex gap-2 mt-3 overflow-x-auto pb-1">
            {plainGallery.map((url, i) => (
              <button
                ...
                className={[
                  'relative w-16 h-16 shrink-0 border overflow-hidden bg-surface-alt',
                  activeImage === i ? 'border-accent/70' : 'border-accent/12',
                ].join(' ')}
              >
                ...
                return (
                  <button
                    ...
                    className={[
                      ...
                      selected ? 'ring-2 ring-focus ring-offset-2 ring-offset-[#0d0d0d]' : '',
                      !available ? 'opacity-25 cursor-not-allowed' : 'hover:scale-110',
                    ].join(' ')}
                    style={{ backgroundColor: hex }}
                  >
                    {!available && (
                      <span className="absolute inset-0 flex items-center justify-center text-[9px] text-ink">✕</span>
                    )}
                  </button>
                )
              })}
      ...
      <div className="flex flex-col">
        ...
        <h1 className="font-['Cormorant_Garamond'] font-light text-ink text-3xl sm:text-4xl leading-tight mb-2">
          {product.name}
        </h1>
        ...
        {product.brand === 'mcs' && (
          <a
            ...
            className="group inline-flex items-center gap-1 w-fit text-[10px] tracking-[0.1em] uppercase
              text-[#5a4c3a] hover:text-accent-text font-['DM_Sans'] transition-colors mb-4 -mt-2"
          >
            Designed by <span className="text-ink-muted group-hover:text-accent-text transition-colors">Hidden Gem</span>
            <span aria-hidden="true">↗</span>
          </a>
        )}

        <div className="flex items-center gap-3 mb-6">
          <span className="font-['Cormorant_Garamond'] text-accent-text text-2xl">
            ${price.toFixed(2)}
          </span>
          {!(activeMaterial?.inStock ?? product.inStock) && (
            <span className="text-[9px] tracking-[0.12em] uppercase px-2 py-0.5
              border border-mist/30 text-ink-muted font-['DM_Sans']">
              Sold Out
            </span>
          )}
        </div>
          ...
          <div className="mb-6">
            ...
            {sizes.length > 10 ? (
              <select
                ...
                className="w-full bg-[#0d0d0d] border border-accent/20 text-ink-muted
              ...
              >
                ...
                {availableSizes.map(s => (
                  <option key={s} value={s} className="bg-surface-alt text-ink">
                    {s}
                  </option>
                ))}
              </select>
        ...
        <div className="flex items-center gap-3 mb-8">
          <div className="flex items-center border border-accent/20">
            <button
              ...
              className="w-9 h-9 text-ink-muted hover:text-accent-text transition-colors"
            >
              −
            </button>
            <span className="w-8 text-center text-ink font-['DM_Sans'] text-sm">{qty}</span>
            <button
          ...
          <button
            ...
            className={[
              ...
              added
                ...
                : 'border-accent/30 text-accent-text hover:bg-accent/08',
        ...
        {product.description && (
          <div
            className="prose-mcs text-sm text-ink-muted font-['DM_Sans'] leading-relaxed
              [&_p]:mb-2 border-t border-accent/10 pt-6"
